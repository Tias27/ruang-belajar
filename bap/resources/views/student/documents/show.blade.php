<x-app-layout title="{{ $document->title }}">
    ...
    <div class="mt-5 flex items-start gap-3 rounded-lg border border-campus-100 bg-campus-50 px-4 py-3 text-sm text-campus-900">
        ...
        <span>Fitur AI di halaman ini hanya memakai konteks materi <strong>{{ $document->title }}</strong>.</span>
    </div>
    ...
    <div class="mt-5 grid gap-5 lg:grid-cols-[1fr_.85fr]">
        <section class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
            <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    <h2 class="font-semibold text-slate-900">Teks yang Terbaca</h2>
    ...
    <div class="mt-5 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <form method="POST" action="{{ route('summaries.store', $document) }}">
            ...
            <button class="flex h-full w-full items-center gap-3 rounded-lg border border-campus-100 bg-campus-700 p-4 text-left text-white shadow-sm hover:bg-campus-900">
                ...
                <span><span class="block text-sm font-semibold">Ringkas Materi</span><span class="mt-1 block text-xs text-campus-100">Singkat, lengkap, poin penting.</span></span>
            </button>
        </form>
        <form method="POST" action="{{ route('chat.create', $document) }}">
            ...
            <button class="flex h-full w-full items-center gap-3 rounded-lg border border-slate-200 bg-white p-4 text-left shadow-sm hover:border-campus-100 hover:bg-campus-50">
                ...
                <span><span class="block text-sm font-semibold text-slate-900">Tanya Materi</span><span class="mt-1 block text-xs text-slate-500">Tanya jawab berbasis materi.</span></span>
            </button>
        </form>
        <form method="POST" action="{{ route('quizzes.store', $document) }}">
            ...
            <button class="flex h-full w-full items-center gap-3 rounded-lg border border-slate-200 bg-white p-4 text-left shadow-sm hover:border-campus-100 hover:bg-campus-50">
                ...
                <span><span class="block text-sm font-semibold text-slate-900">Buat Soal</span><span class="mt-1 block text-xs text-slate-500">10 pilihan ganda dan pembahasan.</span></span>
            </button>
        </form>
        <form method="POST" action="{{ route('flashcards.store', $document) }}">
            ...
            <button class="flex h-full w-full items-center gap-3 rounded-lg border border-slate-200 bg-white p-4 text-left shadow-sm hover:border-campus-100 hover:bg-campus-50">
                ...
                <span><span class="block text-sm font-semibold text-slate-900">Buat Kartu Belajar</span><span class="mt-1 block text-xs text-slate-500">Kartu tanya jawab otomatis.</span></span>
            </button>
        </form>
    </div>
    ...
    <div class="mt-5 grid gap-5 lg:grid-cols-4">
        ...
        <section class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
            <div class="flex items-center justify-between">
                <h2 class="font-semibold text-slate-900">Soal Latihan</h2>
        ...
        <section class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
            <h2 class="font-semibold text-slate-900">Riwayat Tanya Materi</h2>
            <div class="mt-4 space-y-2">
