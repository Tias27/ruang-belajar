<x-app-layout title="Soal Latihan">
    @php
    ...
    <a class="inline-flex items-center gap-1 text-sm font-semibold text-campus-700" href="{{ $quiz->folder ? route('folders.show', $quiz->folder) : route('documents.show', $quiz->document) }}">
        <i data-lucide="arrow-left" class="h-4 w-4"></i> Kembali ke {{ $quiz->folder ? 'folder' : 'dokumen' }}
    </a>

    <section class="mt-4 rounded-lg border border-slate-200 bg-white p-5 shadow-panel">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div>
                <p class="text-sm font-semibold text-campus-700">Kuis Interaktif</p>
                <h1 class="mt-2 text-2xl font-semibold text-campus-900">{{ $quiz->title }}</h1>
                <p class="mt-1 text-sm text-slate-500">{{ $source->title }} • {{ $quiz->questions->count() }} soal pilihan ganda</p>
            </div>
            @if($latestAttempt)
