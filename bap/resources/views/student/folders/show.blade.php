<x-app-layout title="{{ $folder->name }}">
        ...
        <div class="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
            <div class="min-w-0">
                <a href="{{ route('documents.index') }}" class="inline-flex items-center gap-1 text-sm font-semibold text-campus-700">
                    <i data-lucide="arrow-left" class="h-4 w-4"></i> Dokumen
                </a>
                <h1 class="mt-3 break-words text-2xl font-semibold tracking-tight text-campus-900 sm:text-3xl">{{ $folder->name }}</h1>
                <div class="mt-3 flex flex-wrap gap-2 text-xs">
                    <span class="rounded-md bg-accent-50 px-2.5 py-1 font-semibold text-accent-700">Folder Materi</span>
    ...
    <div class="mt-5 flex items-start gap-3 rounded-lg border border-accent-100 bg-accent-50 px-4 py-3 text-sm text-accent-700">
        ...
        <span>Fitur AI di halaman ini membaca gabungan semua file dalam folder <strong>{{ $folder->name }}</strong>. {{ $processedDocuments }}/{{ $folder->documents_count }} file sudah terbaca.</span>
    </div>

    <section class="mt-5 rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <p class="text-sm font-semibold text-campus-700">Mode Belajar Folder</p>
                <h2 class="mt-1 text-xl font-semibold text-campus-900">Belajar dari semua file sebagai satu paket</h2>
            </div>
        ...
        <div class="mt-4 grid gap-3 md:grid-cols-4">
            @foreach([
                ['title' => 'Pahami dulu', 'text' => 'Ringkas semua materi untuk melihat gambaran besar.', 'icon' => 'notebook-tabs'],
                ['title' => 'Tanya detail', 'text' => 'Chat dengan gabungan semua file dalam folder.', 'icon' => 'messages-square'],
                ['title' => 'Uji pemahaman', 'text' => 'Buat latihan campuran dari seluruh paket.', 'icon' => 'list-checks'],
                ['title' => 'Ulang konsep', 'text' => 'Pakai kartu belajar untuk bagian penting.', 'icon' => 'copy-check'],
            ] as $step)
    ...
    <div class="mt-5 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <form method="POST" action="{{ route('folders.summaries.store', $folder) }}">
            @csrf
            <button class="flex h-full w-full items-center gap-3 rounded-lg border border-campus-100 bg-campus-700 p-4 text-left text-white shadow-sm hover:bg-campus-900">
                <i data-lucide="notebook-tabs" class="h-5 w-5 shrink-0"></i>
                <span><span class="block text-sm font-semibold">Ringkas Folder</span><span class="mt-1 block text-xs text-campus-100">Gabungan semua materi.</span></span>
            </button>
        </form>
        <form method="POST" action="{{ route('folders.chat.create', $folder) }}">
            ...
            <button class="flex h-full w-full items-center gap-3 rounded-lg border border-slate-200 bg-white p-4 text-left shadow-sm hover:border-campus-100 hover:bg-campus-50">
                ...
                <span><span class="block text-sm font-semibold text-slate-900">Tanya Folder</span><span class="mt-1 block text-xs text-slate-500">Tanya jawab semua file.</span></span>
            </button>
        </form>
        <form method="POST" action="{{ route('folders.quizzes.store', $folder) }}">
            @csrf
            <button class="flex h-full w-full items-center gap-3 rounded-lg border border-slate-200 bg-white p-4 text-left shadow-sm hover:border-campus-100 hover:bg-campus-50">
                <i data-lucide="list-checks" class="h-5 w-5 shrink-0 text-campus-700"></i>
                <span><span class="block text-sm font-semibold text-slate-900">Buat Soal</span><span class="mt-1 block text-xs text-slate-500">Dari gabungan folder.</span></span>
            </button>
        </form>
        <form method="POST" action="{{ route('folders.flashcards.store', $folder) }}">
            @csrf
            <button class="flex h-full w-full items-center gap-3 rounded-lg border border-slate-200 bg-white p-4 text-left shadow-sm hover:border-campus-100 hover:bg-campus-50">
                <i data-lucide="copy-check" class="h-5 w-5 shrink-0 text-campus-700"></i>
                <span><span class="block text-sm font-semibold text-slate-900">Buat Kartu Belajar</span><span class="mt-1 block text-xs text-slate-500">Kartu dari semua materi.</span></span>
            </button>
        </form>
    </div>
        ...
        <section class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <div class="mt-4 space-y-3 text-sm">
                ...
                <a href="{{ route('folders.flashcards.index', $folder) }}" class="flex items-center justify-between rounded-lg bg-slate-50 p-3 font-medium text-campus-700">
                    <span>{{ $folder->flashcards_count }} kartu belajar folder</span>
                    <i data-lucide="arrow-right" class="h-4 w-4"></i>
                </a>
                <div>
                    <p class="text-xs font-semibold uppercase text-slate-400">Tanya Materi</p>
                    <div class="mt-2 space-y-2">
