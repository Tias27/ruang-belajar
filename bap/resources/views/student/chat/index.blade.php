<x-app-layout title="Riwayat" subtitle="Buka ulang percakapan belajar">
    <section class="rounded-lg border border-slate-200 bg-white p-5 shadow-panel">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
                <p class="text-sm font-semibold text-campus-700">Riwayat</p>
                <h1 class="mt-2 text-2xl font-semibold tracking-tight text-campus-900">Percakapan belajar kamu</h1>
                <p class="mt-1 max-w-2xl text-sm leading-6 text-slate-500">Semua chat dengan dokumen dan folder tersimpan di sini, jadi bisa dibuka ulang kapan saja.</p>
            </div>
            <a href="{{ route('documents.index') }}" class="inline-flex items-center justify-center gap-2 rounded-lg bg-campus-700 px-4 py-3 text-sm font-semibold text-white shadow-sm hover:bg-campus-900">
                <i data-lucide="plus" class="h-4 w-4"></i> Tanya materi baru
            </a>
        </div>
    </section>
    ...
    <section class="mt-5 rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
        <form method="GET" action="{{ route('chat.index') }}" class="flex flex-col gap-3 sm:flex-row">
            <div class="relative min-w-0 flex-1">
                ...
                <input name="q" value="{{ $search }}" placeholder="Cari pertanyaan, jawaban, judul materi, atau folder..." class="pl-9">
            </div>
            ...
            @if($search !== '')
                <a href="{{ route('chat.index') }}" class="inline-flex items-center justify-center gap-2 rounded-lg border border-slate-200 bg-white px-4 py-3 text-sm font-semibold text-slate-700 hover:bg-slate-50">
        ...
        <form method="POST" action="{{ route('chat.bulk-destroy') }}" onsubmit="return confirm('Hapus riwayat yang dipilih? Pesan di dalamnya ikut terhapus.')">
            ...
            @empty
                <div class="rounded-lg border border-dashed border-slate-300 bg-white p-8 text-center lg:col-span-2">
                    ...
                    <p class="mt-1 text-sm text-slate-500">Buka materi, lalu pilih Tanya Materi untuk mulai percakapan.</p>
                </div>
            @endforelse
            </div>
        </form>
