    ...
    @auth
        ...
        <aside class="fixed inset-y-0 left-0 z-40 hidden w-64 border-r border-slate-200 bg-white px-5 py-5 lg:block">
            <a href="{{ auth()->user()->isAdmin() ? route('admin.dashboard') : route('student.dashboard') }}" class="flex items-center gap-3">
                ...
                <span><span class="block text-lg font-semibold tracking-tight">RuangBelajar AI</span><span class="text-xs text-slate-500">{{ $isAdmin ? 'Pusat Pengelolaan' : 'Teman Belajar' }}</span></span>
            </a>
            <div class="mt-6 rounded-lg border {{ $isAdmin ? 'border-campus-100 bg-campus-50 text-campus-900' : 'border-accent-100 bg-accent-50 text-accent-700' }} p-3 text-xs leading-5">
                <span class="font-semibold">{{ $isAdmin ? 'Panel Sistem' : 'Mode Belajar' }}</span>
                <span class="mt-1 block {{ $isAdmin ? 'text-campus-700' : 'text-slate-600' }}">{{ $isAdmin ? 'Pantau aktivitas dan kelola data sistem.' : 'Pilih materi, tanya AI, lalu latihan dari dokumenmu.' }}</span>
            </div>
            <nav class="mt-6 space-y-1 text-sm font-medium">
                ...
                @else
                    ...
                    <x-nav-link href="{{ route('documents.index') }}" icon="library" label="Pustaka Materi" :active="request()->routeIs('documents.index', 'documents.show', 'folders.*', 'summaries.*', 'quizzes.*', 'flashcards.*')" />
                    <x-nav-link href="{{ route('chat.index') }}" icon="messages-square" label="Riwayat" :active="request()->routeIs('chat.*')" />
                    <x-nav-link href="{{ route('documents.create') }}" icon="folder-plus" label="Tambah Materi" :active="request()->routeIs('documents.create')" />
                @endif
            ...
            @unless($isAdmin)
                <a href="{{ route('documents.create') }}" class="absolute inset-x-5 bottom-5 flex items-center gap-3 rounded-lg border border-campus-100 bg-campus-50 p-3 text-sm font-semibold text-campus-900 transition hover:bg-campus-100">
                    ...
                    <span class="min-w-0"><span class="block">Mulai sesi belajar</span><span class="block truncate text-xs font-medium text-slate-500">Unggah file atau folder</span></span>
                </a>
            @endunless
        </aside>
