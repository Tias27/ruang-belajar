    ...
    <div class="mx-auto max-w-2xl py-6">
        ...
        <form method="POST" action="{{ route('register') }}" class="mt-5 grid gap-4 rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <label class="block text-sm font-semibold text-slate-800">Nama pengguna
                <input name="username" value="{{ old('username') }}" required autocomplete="username" placeholder="contoh: Tias123" class="mt-2">
                <span class="mt-1 block text-xs text-slate-500">Gunakan huruf, angka, tanda hubung, atau garis bawah. Minimal 3 karakter.</span>
            </label>
