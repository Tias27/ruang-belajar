        ...
        return match (true) {
            str_contains($lower, 'quota')
                ...
                || str_contains($lower, 'prepayment credits are depleted')
                    => 'Gemini belum bisa dipakai saat ini karena kuota atau saldo API sedang bermasalah.',
            str_contains($lower, 'unavailable')
                || str_contains($lower, 'high demand')
                || str_contains($lower, '503')
                    => 'Gemini sedang padat, jadi jawaban AI penuh belum bisa dibuat saat ini.',
            str_contains($lower, 'api key')
                || str_contains($lower, 'permission_denied')
                || str_contains($lower, '401')
                || str_contains($lower, '403')
                    => 'Gemini belum bisa dipakai saat ini karena konfigurasi API belum valid.',
            str_contains($lower, 'timeout')
                || str_contains($lower, 'timed out')
                || str_contains($lower, 'curl error')
                || str_contains($lower, 'koneksi')
                    => 'Gemini belum bisa dipakai saat ini karena koneksi server sedang gagal atau terlalu lama.',
            default => 'Gemini belum bisa dipakai saat ini.',
        };
    }
}
