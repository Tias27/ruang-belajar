<x-app-layout title="Materi Saya" subtitle="Buka materi untuk ringkas, tanya AI, latihan, atau flashcard">
    <section class="rounded-lg border border-slate-200 bg-white p-5 shadow-panel">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
                <p class="text-sm font-semibold text-campus-700">Materi Saya</p>
                <h1 class="mt-2 text-2xl font-semibold tracking-tight text-campus-900">Pilih materi dulu</h1>
                <p class="mt-1 max-w-2xl text-sm leading-6 text-slate-500">Folder = banyak file digabung. File satuan = satu dokumen dipelajari sendiri.</p>
            </div>
            <a href="{{ route('documents.create') }}" class="inline-flex items-center justify-center gap-2 rounded-lg bg-campus-700 px-4 py-3 text-sm font-semibold text-white shadow-sm hover:bg-campus-900">
                <i data-lucide="folder-plus" class="h-4 w-4"></i> Upload Materi
            </a>
        </div>
    </section>
    ...
    <section class="mt-5 rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
        <form method="GET" action="{{ route('documents.index') }}" class="flex flex-col gap-3 sm:flex-row">
            <div class="relative min-w-0 flex-1">
                ...
                <input name="q" value="{{ $search }}" placeholder="Cari nama materi atau isi file..." class="h-12" style="padding-left: 2.5rem;">
            </div>
            ...
            @if($search !== '')
                <a href="{{ route('documents.index') }}" class="inline-flex h-12 min-w-28 shrink-0 items-center justify-center gap-2 whitespace-nowrap rounded-lg border border-slate-200 bg-white px-4 text-sm font-semibold text-slate-700 hover:bg-slate-50">
    ...
    @if($folders->count() > 0)
        <section class="mt-5">
            <div class="mb-3 flex items-center justify-between">
                <h2 class="text-sm font-semibold text-slate-900">Folder Gabungan</h2>
    ...
    <section class="mt-5" x-data="{
        ...
        <div class="mb-3 flex items-center justify-between">
            <h2 class="text-sm font-semibold text-slate-900">File Satuan</h2>
                        ...
                        <div class="grid grid-cols-2 gap-2 sm:flex sm:flex-wrap sm:justify-end">
                            ...
                            <form method="POST" action="{{ route('chat.create', $document) }}">
                                ...
                                <button class="inline-flex h-9 w-full items-center justify-center gap-1 rounded-lg border border-campus-100 bg-campus-50 px-3 text-xs font-semibold text-campus-700 hover:bg-campus-100">
                                    <i data-lucide="messages-square" class="h-3.5 w-3.5"></i> Tanya AI
                                </button>
                            </form>
                            ...
                            <form method="POST" action="{{ route('flashcards.store', $document) }}">
                                @csrf
                                <button class="inline-flex h-9 w-full items-center justify-center gap-1 rounded-lg border border-slate-200 bg-white px-3 text-xs font-semibold text-slate-700 shadow-sm hover:bg-slate-50">
                                    <i data-lucide="copy-check" class="h-3.5 w-3.5"></i> Flashcard
                                </button>
                            </form>
