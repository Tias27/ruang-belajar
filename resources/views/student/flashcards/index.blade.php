<x-app-layout title="Flashcard">
    ...
    <section class="mt-4 rounded-lg border border-slate-200 bg-white p-5 shadow-panel">
        <p class="text-sm font-semibold text-campus-700">Flashcard</p>
        ...
        @forelse($flashcards as $flashcard)
            ...
            <article x-data="{open:false}" class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
                ...
                <p class="mt-3 text-xs text-slate-500">
                    ...
                    @if($flashcard->last_reviewed_at)                        - terakhir {{ $flashcard->last_reviewed_at->diffForHumans() }}
                    @endif
                </p>
            </article>
        @empty
            <p class="rounded-lg border border-dashed border-slate-300 bg-white p-6 text-sm text-slate-500">Belum ada flashcard.</p>
        @endforelse
    </div>
</x-app-layout>
